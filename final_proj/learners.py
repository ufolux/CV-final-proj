...
class DiscLearner:
    ...
    def learn_one_batch(self, storage):
        while not storage.is_buffer_ready():
            time.sleep(0.1)

        # image_batch: (batch, 64, 64, 3)
        image_batch = storage.get_final_render_batch()
        ...
        return loss, d_fake, d_real, d_std, d_mean, real_sample, fake_sample
...
class PolicyLearner:
    ...
    def get_rewards(self, final_renders, storage, gamma=0.99):
        # final_renders: (batch, 64, 64, 3)

        info = storage.get_info(['d_std', 'd_mean', 'd_weights'])
    ...
    def learn_one_batch(self, storage, discriminator_model):
        while not storage.is_queue_ready():
            time.sleep(0.1)
        traj_batch = storage.get_trajectory_batch()

        # action_batch should be a dict, each value is (batch, time, size[i]), where i comes from LOCATION_KEYS
        image_batch, action_batch, prev_action_batch, action_masks, value_batch, noise_samples, final_renders = traj_batch

        image_batch = np.array(image_batch)
        # final_renders = image_batch[:, -1, :, :, :] # (batch, 64, 64, 3)
        final_renders = np.array(final_renders)
        reward_batch = self.get_rewards(final_renders, storage) # (batch,)
        ...
        adv_batch = self.get_adv(reward_batch, value_batch) # (batch, time)
        ...
        if results is None: # numerical issue
            ...
            self.optimizer.step()
            return total_loss, policy_loss, value_loss, entropy_loss, reward_mean, value_mean, neg_log, final_renders
